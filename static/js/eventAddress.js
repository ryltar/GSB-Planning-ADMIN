var grid = $("#grid-address").bootgrid({
  ajax: true,
  rowSelect: true,
  labels:{
...
}).on("loaded.rs.jquery.bootgrid", function()
{
  grid.find(".command-edit").on("click", function(e)
  {
    ...
    if($(this).data("row-id") > 0) {
      ...
      $('#edit_idaddr').val(ele.siblings(':first').html());
      $('#edit_num').val(ele.siblings(':nth-of-type(2)').html());
      $('#edit_street').val(ele.siblings(':nth-of-type(3)').html());
      $('#edit_codepost').val(ele.siblings(':nth-of-type(4)').html());
      $('#edit_city').val(ele.siblings(':nth-of-type(5)').html());
      $('#edit_country').val(ele.siblings(':nth-of-type(6)').html());
      $('#edit_indication').val(ele.siblings(':nth-of-type(7)').html());
